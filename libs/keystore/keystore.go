...
// PrivateKey searches the key store for a given keyID and returns the private key.
func (ks *KeyStore) PrivateKey(keyID string) (string, error) {
	key, found := ks.store[keyID]
	if !found {
		return "", errors.New("keyID lookup failed")
	}

	return key.privatePEM, nil
}

// PublicKey searches the key store for a given keyID and returns the public key.
func (ks *KeyStore) PublicKey(keyID string) (string, error) {
	key, found := ks.store[keyID]
	if !found {
		return "", errors.New("keyID lookup failed")
	}
...
// Example: /zarf/keys/60877A3C-9AB6-4A50-9F27-B56D78229D92.pem
...
// toPublicPEM takes a private key in PEM (Privacy Enhanced Mail) format
// as input and converts it into its corresponding public key in PEM format
